...
{
    private readonly MinioClient client = new MinioClient().WithEndpoint(options.Value.Endpoint).WithCredentials(options.Value.AccessKey, options.Value.SecretKey).WithSSL(options.Value.UseSSL).Build();
    ...
    public Task<string> GetUrlAsync(string objectKey, CancellationToken cancellationToken) => client.PresignedGetObjectAsync(new PresignedGetObjectArgs().WithBucket(options.Value.BucketName).WithObject(objectKey).WithExpiry(options.Value.UrlExpirationSeconds));
}
